...
- **Comfort scoring:** `HourlyForecast.aqi` is the EPA 0–500 value (stamped per hour from AirNow). `enhancedCyclingComfort` weighs it by EPA category and hard-caps the score when air is officially bad (≥ 151 → max 40%, ≥ 301 → max 15%); its temperature component scores against the NWS heat index when that applies. Analytics ("best hour to start") must filter out past hours — Apple's hourly feed includes earlier hours of the current day.
