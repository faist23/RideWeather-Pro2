...
class StravaService: NSObject, ObservableObject, ASWebAuthenticationPresentationContextProviding {
    ...
    func extractRouteFromStravaRoute(routeId: Int) async throws -> (coordinates: [CLLocationCoordinate2D], totalDistanceMeters: Double, elevationAnalysis: ElevationAnalysis?) {
        print("StravaService: Fetching route detail for route \(routeId)")
        
        // 1. Fetch route streams to get elevation data
        // We try to fetch streams first to get accurate elevation data.
        // If streams fail (e.g. route is too simple?), we fall back to detail polyline.
        
        do {
            let routeStreams = try await fetchRouteStreams(routeId: routeId)
            
            guard let latlngData = routeStreams.latlng?.data,
                  let distanceData = routeStreams.distance?.data else {
                throw StravaError.noRouteData
            }
            
            var coordinates: [CLLocationCoordinate2D] = []
            for pair in latlngData {
                if pair.count == 2 {
                    coordinates.append(CLLocationCoordinate2D(latitude: pair[0], longitude: pair[1]))
                }
            }
            
            let totalDistance = distanceData.last ?? 0.0
            
            var elevationAnalysis: ElevationAnalysis?
            if let altitudeData = routeStreams.altitude?.data, !altitudeData.isEmpty {
                elevationAnalysis = makeElevationAnalysis(distance: distanceData, altitude: altitudeData)
            }
            
            print("StravaService: Extracted \(coordinates.count) GPS points from route streams")
            return (coordinates, totalDistance, elevationAnalysis)
            
        } catch {
            print("StravaService: Failed to fetch route streams, falling back to polyline. Error: \(error)")
            
            // Fallback: Fetch route detail and decode polyline
            let routeDetail = try await fetchRouteDetail(routeId: routeId)
            let totalDistance = routeDetail.distance
            ...
            print("StravaService: Extracted \(coordinates.count) GPS points from route polyline (no elevation)")
            return (coordinates, totalDistance, nil)
        }
    }
    
    /// Fetches streams for a specific Route
    private func fetchRouteStreams(routeId: Int) async throws -> StravaStreams {
        await refreshTokenIfNeededAsync()
        guard let accessToken = currentTokens?.accessToken else { throw StravaError.notAuthenticated }
    ...
    func extractRouteFromActivity(activityId: Int) async throws -> (coordinates: [CLLocationCoordinate2D], totalDistanceMeters: Double, elevationAnalysis: ElevationAnalysis?) {
            await refreshTokenIfNeededAsync()
            ...
            for i in 0..<timeData.count {
                ...
                if let alt = altitudeData?[safe: i] {
                    filteredAltitudes.append(alt)
                }
            }
            ...
            // Build ElevationAnalysis if we have enough data
            var elevationAnalysis: ElevationAnalysis?
            if !filteredAltitudes.isEmpty && filteredAltitudes.count == filteredDistances.count {
                elevationAnalysis = makeElevationAnalysis(distance: filteredDistances, altitude: filteredAltitudes)
            }
            
            print("StravaService: Extracted \(filteredCoordinates.count) MOVING GPS points and \(totalDistance)m from activity")
            
            // Return the filtered coordinates, correct distance, and elevation analysis
            return (coordinates: filteredCoordinates, totalDistanceMeters: totalDistance, elevationAnalysis: elevationAnalysis)
        }
    
    // MARK: - Elevation Analysis Helper
    private func makeElevationAnalysis(distance: [Double], altitude: [Double]) -> ElevationAnalysis {
        ...
        for i in 0..<distance.count {
            ...
            var grade: Double?
            if i > 0 {
                let distDiff = d - distance[i-1]
                if distDiff > 0 {
                    grade = ((a - altitude[i-1]) / distDiff) * 100
                }
            }
            
            profile.append(ElevationPoint(distance: d, elevation: a, grade: grade))
            prevAlt = a
        }
        
        let minEl = altitude.min() ?? 0
        let maxEl = altitude.max() ?? 0
