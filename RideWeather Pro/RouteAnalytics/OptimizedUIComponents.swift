...
struct ImprovedExportToGarmin: View {
    ...
    var body: some View {
        VStack(spacing: 12) {
            Button(action: {
                ...
                HStack(spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text(garminService.isAuthenticated ?
                             "Push route to your Garmin Connect" :
            ...
            .disabled(!garminService.isAuthenticated || exportingToGarmin)
            .buttonStyle(.plain)
        ...
        .alert("Upload Successful!", isPresented: $showSuccessAlert) {
            ...
            Text("Your course has been uploaded to Garmin Connect. Sync your device to see it.")
        }
    }
    ...
    private func exportToGarmin() async {
        ...
        guard let controller = viewModel.advancedController else {
            print("📱 UI: ❌ No advanced controller")
            await MainActor.run {
                exportError = "No workout data available. Generate a pacing plan first."
                exportingToGarmin = false
            }
            return
        }
        
        guard let pacingPlan = viewModel.finalPacingPlan else {
            print("📱 UI: ❌ No pacing plan")
            await MainActor.run {
                exportError = "No pacing plan available. Generate a plan first."
                exportingToGarmin = false
            }
            return
        }
        
        guard !viewModel.enhancedRoutePoints.isEmpty else {
            print("📱 UI: ❌ No route points")
            await MainActor.run {
                exportError = "No route data available. Import a route first."
                exportingToGarmin = false
            }
            return
        }
        ...
        print("📱 UI: ✅ Prerequisites validated")
        print("📱 UI: Route points: \(viewModel.enhancedRoutePoints.count)")
        print("📱 UI: Course name: \(viewModel.routeDisplayName)")
        ...
        do {
            ...
            print("📱 UI: Final course name: '\(courseName)'")
            ...
            guard let data = fitData else {
                print("📱 UI: ❌ FIT generation returned nil")
                throw GarminService.GarminError.invalidResponse
            }
            ...
            print("📱 UI: ✅ Upload completed successfully!")
            ...
            await MainActor.run {
                if let garminError = error as? GarminService.GarminError {
                    exportError = "Upload failed: \(garminError.localizedDescription)"
                } else {
