    ...
    var body: some View {
        NavigationView {
            ...
            .fileImporter(
                ...
                viewModel.handleFileImport(result: result, settings: settings)
            }
            ...
            .sheet(isPresented: $viewModel.showingExportOptions) {
                ExportOptionsView(analysis: viewModel.currentAnalysis!)
            }
        }
    }
        ...
        ScrollView {
            VStack(spacing: 20) {
                if let analysis = viewModel.currentAnalysis {
                    ...
                    // Quick Stats
                    QuickStatsCard(analysis: analysis, useMetric: settings.useMetric)
...
struct PowerZoneDistributionCard: View {
    ...
    private let zoneNames = ["Z1: Recovery", "Z2: Endurance", "Z3: Tempo",
                            "Z4: Threshold", "Z5: VO2 Max", "Z6: Anaerobic"]
...
class RideAnalysisViewModel: ObservableObject {
    ...
    func handleFileImport(result: Result<[URL], Error>, settings: AppSettings) {
        switch result {
        case .success(let urls):
            guard let url = urls.first else { return }
            analyzeFile(at: url, settings: settings)
        case .failure(let error):
            print("File import error: \(error)")
        }
    }
    
    func analyzeFile(at url: URL, settings: AppSettings) {
        isAnalyzing = true
