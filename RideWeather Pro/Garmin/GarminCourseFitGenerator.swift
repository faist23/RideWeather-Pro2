...
struct GarminCourseFitGenerator {
    ...
    private func writeCoursePointsWithPowerAndTimeTargets(
        encoder: Encoder,
        routePoints: [EnhancedRoutePoint],
        pacingPlan: PacingPlan,
        settings: AppSettings,
        ...
        print("📍 Writing course points with power targets and time checkpoints...")
        
        var messageIndex: UInt16 = 0
        
        // 1. Write the power target course points at segment boundaries
        for (segmentIndex, segment) in pacingPlan.segments.enumerated() {
            ...
            try coursePointMesg.setFavorite(false)
            
            encoder.write(mesg: coursePointMesg)
        }
        
        // 2. Write time checkpoint course points (if enabled)
        try writeTimeCheckpoints(
            encoder: encoder,
            routePoints: routePoints,
            pacingPlan: pacingPlan,
            settings: settings,
        ...
        // 3. Nutrition Cues
        if let strategy = fuelingStrategy {
            try writeNutritionCues(
                encoder: encoder,
                routePoints: routePoints,
                pacingPlan: pacingPlan,
                strategy: strategy,
    ...
    private func writeNutritionCues(
        encoder: Encoder,
        routePoints: [EnhancedRoutePoint],
        pacingPlan: PacingPlan,
        strategy: FuelingStrategy,
        messageIndex: inout UInt16
    ) throws {
        
        for fuelPoint in strategy.schedule {
            // Convert time (minutes) to distance (meters) using PacingPlan
            let targetTimeSeconds = fuelPoint.timeMinutes * 60
            let targetDistance = calculateDistanceAtTime(targetTime: targetTimeSeconds, pacingPlan: pacingPlan)
            
            // Find closest route point
            guard let closestPoint = routePoints.min(by: {
            ...
            let latSemi = Int32(closestPoint.coordinate.latitude * (Double(Int32.max) / 180.0))
            let lonSemi = Int32(closestPoint.coordinate.longitude * (Double(Int32.max) / 180.0))
            try cueMesg.setPositionLat(latSemi)
            try cueMesg.setPositionLong(lonSemi)
            try cueMesg.setDistance(closestPoint.distance)
            
            // Label: "DRINK" or "EAT 40g"
            // Keep it short for head units (max 10-15 chars usually)
            let label: String
            switch fuelPoint.fuelType {
            case .drink, .electrolytes:
                label = "DRINK"
            default:
                // Extract number from "40g carbs" -> "EAT 40g"
                let amountStr = fuelPoint.amount.components(separatedBy: " ").first ?? ""
                label = "EAT \(amountStr)"
            }
            
            try cueMesg.setName(String(label.prefix(15)))
            
            // Use .food type if available, otherwise .generic or .water
            if fuelPoint.fuelType == .drink {
                try cueMesg.setType(.water)
            } else {
                try cueMesg.setType(.food)
            }
            
            encoder.write(mesg: cueMesg)
        }
    }
    
    // Helper to inverse the time/distance calculation
    private func calculateDistanceAtTime(targetTime: Double, pacingPlan: PacingPlan) -> Double {
        ...
        return currentDistance // Should handle end of ride
    }
    ...
    /// Writes time checkpoint course points at specified distances
    private func writeTimeCheckpoints(
        encoder: Encoder,
        routePoints: [EnhancedRoutePoint],
        pacingPlan: PacingPlan,
        settings: AppSettings,
        ...
        guard settings.enableTimeCheckpoints else {
            print("⏱️ Time checkpoints disabled in settings")
            return
        }
        ...
        while checkpointDistance < totalDistance {
            ...
            // Position
            let latSemi = Int32(checkpointPoint.coordinate.latitude * (Double(Int32.max) / 180.0))
            let lonSemi = Int32(checkpointPoint.coordinate.longitude * (Double(Int32.max) / 180.0))
            try coursePointMesg.setPositionLat(latSemi)
            try coursePointMesg.setPositionLong(lonSemi)
            ...
            try coursePointMesg.setName(String(checkpointName.prefix(15)))
            
            // Point type - use generic for checkpoints
            try coursePointMesg.setType(.generic)
            try coursePointMesg.setFavorite(false)
